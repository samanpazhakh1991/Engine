...
namespace SpecFlowTest
{
    ...
    {
        static readonly ConcurrentDictionary<string, ICircuitBreaker> connections = new();
        ...
        public static async Task Execute(string url, HttpMethod httpMethod)
        {
            await Execute<object>(url, httpMethod);
        }
        public static async Task<TOutput> Execute<TOutput>(string url, HttpMethod httpMethod)
        {
            return await Execute<object, TOutput>(url, httpMethod);
        }
        public static async Task<TOutput> Execute<TInput, TOutput>(string url, HttpMethod httpMethod, TInput input = default(TInput))
        {
            var cb = connections.First(i => url.StartsWith(i.Key)).Value;
            return await cb.ExecuteService(input, i => _execute<TInput, TOutput>(url, httpMethod, i));
        }
        private static async Task<TOutput> _execute<TInput, TOutput>(string url, HttpMethod httpMethod, TInput input = default(TInput))
        {
